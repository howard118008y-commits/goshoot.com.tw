...
(function () {
  ...
  var FAQ = [
    ...
    [["機率", "公平", "作弊", "驗證"], "每套賞的賞級數量、剩餘與機率都公開，開獎用 commit-reveal＋公開亂數（drand），到 " + APP + "/verify 可以自己驗算。"],
  ...
  var css = ".gsa{position:fixed;right:18px;bottom:18px;z-index:2000;font-family:inherit}" +
    ...
    ".gsa-head{display:flex;align-items:center;gap:8px;padding:13px 16px;color:#ECECEA;font-size:15px;border-bottom:1px solid rgba(255,255,255,.10)}" +
    ".gsa-dot{width:8px;height:8px;border-radius:50%;background:#B6F500;animation:gsaPulse 1.4s ease-in-out infinite}" +
    ".gsa-sub{color:#8A8A92;font-size:12px}" +
    ".gsa-x{margin-left:auto;background:none;border:none;color:#8A8A92;font-size:14px;cursor:pointer}" +
    ...
    ".gsa-msg.assistant a{color:#B6F500;font-weight:700}" +
    ".gsa-msg.user{align-self:flex-end;background:rgba(182,245,0,.16);color:#EFFFD0;border:1px solid rgba(182,245,0,.25);border-bottom-right-radius:4px}" +
    ...
    ".gsa-quick button:hover{color:#B6F500;border-color:rgba(182,245,0,.4)}" +
    ...
    ".gsa-input input:focus{border-color:rgba(182,245,0,.45)}" +
    ".gsa-input button{background:linear-gradient(135deg,#B6F500,#9BD400);color:#0B0B0D;font-weight:800;border:none;border-radius:12px;padding:0 16px;font-size:13.5px;cursor:pointer}" +
    "@keyframes gsaPulse{0%,100%{opacity:1}50%{opacity:.35}}" +
    "@media (max-width:480px){.gsa{right:12px;bottom:12px}.gsa-panel{bottom:64px}}";
